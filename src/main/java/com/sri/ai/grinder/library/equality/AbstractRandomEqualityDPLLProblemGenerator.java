...
/**
 * An iterator of random DPLL-style equality problem expressions, generated according to certain parameters
 * (see {@link #AbstractRandomDPLLProblemGenerator(Random, int, int, int, int, int)} for their description).
 * 
 * @author braz
 *
 */
public abstract class AbstractRandomEqualityDPLLProblemGenerator extends EZIterator<Expression> {
	...
	/**
	 * Extensions must implement this abstract method to create a quantified expressions based on a given formula and indices
	 * (the quantified variables). Other variables in the formula are considered free variables.
	 * @param formula
	 * @param indices
	 * @return
	 */
	 ...
	 */
	public AbstractRandomEqualityDPLLProblemGenerator(Random random, int numberOfVariables, int numberOfConstants, int minimumNumberOfIndices, int depth, int breadth) {
		super();
		this.random = random;
