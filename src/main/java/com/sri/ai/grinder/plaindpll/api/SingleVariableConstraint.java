...
public interface SingleVariableConstraint extends Expression {
	...
	/**
	 * The result of applying a new constraint to a {@link SingleVariableEqualityConstraint},
	 * containing the new constraint and a collection of implied literals
	 * that cannot be represented by the new constraint (for not being on its constrained variable).
	 * @author braz
	 *
	 */
	static interface ApplicationResult {
		SingleVariableConstraint getConstraint();
		Collection<Expression> getImpliedLiterals();
	}
	
	static class DefaultApplicationResult implements ApplicationResult {
		private SingleVariableConstraint constraint;
		private Collection<Expression> impliedLiterals;

		public DefaultApplicationResult(SingleVariableConstraint constraint, Collection<Expression> impliedLiterals) {
			super();
		...
		@Override
		public Collection<Expression> getImpliedLiterals() {
	...
	/**
	 * Returns an {@link ApplicationResult} containing a constraint on the same variable as this one,
	 * and a collection of literals.
	 * The conjunction of the new constraint and the implied literals must be
	 * equivalent to the conjunction of this constraint and the signed splitter.
	 * @param literal the splitter
	 * @param process the rewriting process
	 * @return the application result or <code>null</code> if contradiction.
	 */
	ApplicationResult incorporate(Expression literal, RewritingProcess process);
...
//	SingleVariableEqualityConstraint clone();
//	
...
//	 */
//	public abstract SingleVariableEqualityConstraint incorporate(boolean splitterSign, Expression splitter, RewritingProcess process);
//
//	/** Same as {@link #incorporate(boolean, Expression, RewritingProcess)} but using {@link SignedSplitter}. */
//	default SingleVariableEqualityConstraint incorporate(SignedSplitter signedSplitter, RewritingProcess process) {
...
//	 * This violates the immutability assumption about {@link Expression} and {@link SingleVariableEqualityConstraint}
...
//	 */
//	default SingleVariableEqualityConstraint project(Collection<Expression> eliminatedIndices, RewritingProcess process) {
...
//		SingleVariableEqualityConstraint result = ExpressionConstraint.wrap(getConstraintTheory(), getSupportedIndices(), resultExpression);
//		return result;
//	}
//
//	/**
