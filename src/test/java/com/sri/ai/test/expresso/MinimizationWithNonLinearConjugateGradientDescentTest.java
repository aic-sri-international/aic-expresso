...
import com.sri.ai.expresso.minimization.MinimizationWithNonlinearConjugateGradientDescent;
...
public class MinimizationWithNonLinearConjugateGradientDescentTest {
	...
	public void testMinOneVariable() {
		...
		Assert.assertTrue(minAreEquals(0.5, "0.5*(1+X^2)^(1/3)", new double[] {-16537}));
		Assert.assertTrue(minAreEquals(-2.2129819407, "3*X^4 + X^3 - 6*X^2 + 2", new double[] {-10}));
		Assert.assertTrue(minAreEquals(-2.2129819407, "3*X^4 + X^3 - 6*X^2 + 2", new double[] {-1000}));
	}
	...
	/**
	 * Method to test if the minimum given by the function is close (precision can be chosen) to the real result.
	 *
	 */
	public static boolean minAreEquals(double expectedResult, String stringToMinimize, double[] initialGuess) {
		MinimizationWithNonlinearConjugateGradientDescent minWithNonlinearCGD = new MinimizationWithNonlinearConjugateGradientDescent(Expressions.parse(stringToMinimize), initialGuess);
		double minimumOfExpression = minWithNonlinearCGD.findMinimum();
	...
	/**
	 * Method to test if the argmin given by the function is close (precision can be chosen) to the real result.
	 *
	 */
	public static boolean argminAreEquals(double[] expectedResult, String stringToMinimize, double[] initialGuess) {
		MinimizationWithNonlinearConjugateGradientDescent minWithNonlinearCGD = new MinimizationWithNonlinearConjugateGradientDescent(Expressions.parse(stringToMinimize), initialGuess);
		double[] argminOfExpression = minWithNonlinearCGD.findArgmin();
